...
if __name__ == "__main__":
    ...
    detr = detr_resnet50(pretrained=False,num_classes=90+1).eval()  # <------这里类别需要+1
    state_dict =  torch.load('./outputs_mix_mot_ch/checkpoint.pth')   # <-----------修改加载模型的路径
    detr.load_state_dict(state_dict["model"])
    detr.to(device)
    

    files = os.listdir("./outputs_mix_mot_ch/img1")

    for file in files:
        img_path = os.path.join("./outputs_mix_mot_ch/img1",file)
