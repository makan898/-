...
def plot_result(pil_img, prob, boxes,save_name=None,imshow=False, imwrite=False):
    ...
    for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes):
        ...
        cl = p.argmax()
        label_text = '{}: {}%'.format(LABEL[cl],round(p[cl].cpu().numpy()*100,2))
    ...
    if imwrite:
        ...
        cv2.imwrite('./result/pred/{}.jpg'.format(save_name), opencvImage)
...
if __name__ == "__main__":
    ...
    state_dict =  torch.load('./outputs_mix_mot_ch/checkpoint.pth')   # <-----------修改加载模型的路径
    ...
    for i in range(video_frame_cnt):
        ...
        image = plot_result(image_pil, scores, boxes,save_name=i,imshow=False, imwrite=True)
